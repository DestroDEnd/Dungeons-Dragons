...
5. If the narrative dictates the player finds an item, add it to 'inventoryAdd'. If an item breaks, is dropped, or is lost (even temporarily), add it to 'inventoryRemove'. CRITICAL: If you remove an item from the inventory for ANY reason, you MUST explicitly narrate what happened to it (e.g. "it slips from your grasp into the mud", "it shatters") so the player understands they no longer have it.
6. PLAYER HEALTH ONLY: 'hpChange' ONLY applies to the PLAYER'S HP. If the PLAYER gets hurt, output a negative number (e.g. -2). If the PLAYER heals, output a positive number. If the PLAYER deals damage to an ENEMY, DO NOT modify 'hpChange' (output 0). CRITICAL: Do NOT lower the player's HP unless they explicitly take physical damage or are attacked in the narrative text.
